function output = dfield6(action,input1,input2,input3)

% DFIELD6	is an interactive tool for studying single first order
%	differential equations.  When DFIELD6 is executed, a DFIELD6 Setup
...
%	mouse button is depressed in the DFIELD6 Display window, the
...
%  Last modified: April 2, 2003
  ...
  if strcmp(action,'initialize')
    ...
    % First we make sure that there is no other copy of DFIELD6
    % running, since this causes problems.	
    
    dfh = findobj('tag','DFIELD6');
    if ~isempty(dfh);
      qstring = {'There are some open DFIELD6 figures (perhaps invisible).  ';...
		 'What do you want to do?'};
      tstring = 'Only one copy of DFIELD6 can be open at one time.';
      b1str = 'Restart DFIELD6.';
    ...
    % Next we look for old files created by DFIELD6.
	...
	ll = fgetl(fid);
	fclose(fid);
	if strcmp(ll,'%% Created by DFIELD6')
	  delete(fn)
	else
	  kk = [kk;k];
	end
    ...
    dfset = figure('name','DFIELD6 Setup','numbertitle','off',...
		   'tag','DFIELD6','visible','off',...
		   'user',ud);
    
    dfield6('figdefault',dfset);
  ...
  meproceed = uimenu(mefile,'label','Proceed','call',...
    ...
    meadd = uimenu(sysmenu,'label','Add current equation to the gallery',...
		   'call','dfield6(''addgall'');','tag','add system');
    sep = 'on';
  ...
  elseif strcmp(action,'proceed')
    ...
    if any(isnan(WINvect))
      errmsg = ['One of the entries defining the display window ',...
		'is not a number.'];
      fprintf('\a')
      errordlg(errmsg,'DFIELD6 error','on');
      return
    end
    xstr = sud.c.xname;
    if isempty(xstr)
    ...
    if (sud.flag == 1)
      dfdisp = findobj('name','DFIELD6 Display');
      dud = get(dfdisp,'user');
	...
	if ~isempty(pn)
	  if isempty(pv) 
	    ...
	    xderivstr = dfield6('paraeval',pn,pv,derivstr);
	...
	return
      ...
      % Find DFIELD6 Display if it exists.	
      % If DFIELD6 Display exists, update it.  If it does not build it.
	...
	dfdisp = figure('name','DFIELD6 Display',...
			'numbertitle','off','interrupt','on',...
			'visible','off','tag','DFIELD6');
	dfield6('figdefault',dfdisp);
	dud = get(dfdisp,'user');	
	dud.syst = sud.c;
	dud.settings = sud.settings;
	fs = dud.fontsize;
	...
	merestart = uimenu(fmenu,'label','Restart DFIELD6','call',...
			   'dfield6(''restart'')','separator','on','pos',8);
	
	mequit = uimenu(fmenu,'label','Quit DFIELD6',...
			'call','dfield6(''quit'')','separator','on');
	...
	solverstr = ['ud = get(gcf,''user'');',...
		     ...
		     'set(me,''checked'',''on'');',...
		     'set(gcf,''user'',ud);',...
		     'dfset = findobj(''name'',''DFIELD6 Setup'');',...
		     'sud = get(dfset,''user'');',...
	...
	meset   = uimenu(menu(1),'label','Window settings.',...
			 'call','dfield6(''settings'')');
	
	menu(6) = uimenu(menu(1),'label','Make the Display Window inactive.',...
			 'call','dfield6(''hotcold'')','separator','on');
	
	% View menu
	
	set(findobj(gcf,'label','&View'),'pos',4);
	set(findobj(gcf,'label','&Figure Toolbar'),...
	    'call','dfield6(''showbar'')');
	dud.menu = menu;
	
	set(dfdisp,'ToolBar','none');
	set(0,'showhiddenhandles',hhsetup);
	set(gcf,'WindowButtonDownFcn','dfield6(''down'')');
	set(dfdisp,'WindowButtonMotionFcn','dfield6(''cdisp'')');
	...
	ppkbd = findobj('name','DFIELD6 Keyboard input','vis','on');
	if ~isempty(ppkbd),dfield6('kbd'),end
	dfield6('dirfield',dfdisp);	
    end
  ...
  elseif strcmp(action,'dirfield')
    ...
    % Find DFIELD6 Display and get the user data.
  ...
  elseif strcmp(action,'hotcold')
    ...
    if (findstr(get(mehc,'label'),'inactive'))
      ...
      set(dfdisp,'WindowButtonDownFcn','dfield6(''down'')');
  ...
  elseif strcmp(action,'several')
    ...
    %  set([ch;mh],'enable','on');
    set(ch,'enable','on');
    set(disph,'WindowbuttonDownFcn','dfield6(''down'')');
  ...
  elseif strcmp(action,'kcompute')
    ...
    % button on the DFIELD6 Keyboard figure.
	...
	if isempty(pv) 
	  ...
	  xstr = dfield6('paraeval',pn,pv,xstr);
	  tstr = dfield6('paraeval',pn,pv,tstr);
	end
      end
    end
    xvalue = str2num(xstr);
    ...
    if compute
      dfield6('solution',initpt,dfdisp);
    end
    nstr = get(dud.notice,'string');
    nstr(1:4) = nstr(2:5);
    nstr{5} = 'Ready.';
  ...
  elseif strcmp(action,'kbd')
    ...
    % sets up the DFIELD6 Keyboard figure which allows accurate input of 
    % initial values using the keyboard.
    
    ppkbd = findobj('name','DFIELD6 Keyboard input');
    if ~isempty(ppkbd)
      delete(ppkbd);
    end    
    dfdisp = gcf;
    ...
    ppkbd = figure('name','DFIELD6 Keyboard input',...
		   'vis','off',...
		   'numbertitle','off','tag','DFIELD6');
    
    dfield6('figdefault',ppkbd);
    ...
    kbd.comp = uicontrol('style','push',...		
			 'string','Compute','call','dfield6(''kcompute'')');
  ...
  elseif strcmp(action,'zoomin')
    ...
    set(gcf,'WindowButtonDownFcn','dfield6(''zoom'')',...
  ...
  elseif strcmp(action,'solvset')
    ...
    setfig = findobj('name','DFIELD6 Solver settings');
    if ~isempty(setfig)
    ...
    setfig = figure('name','DFIELD6 Solver settings','numbertitle','off',...
		    'tag','DFIELD6','vis','off');
    
    dfield6('figdefault',setfig);
  ...
  elseif strcmp(action,'settings')
    ...
    % up the DFIELD6 Settings window, which allows the user to interactively
    ...
    setfig = findobj('name','DFIELD6 Window settings');
    if ~isempty(setfig)
      close(setfig);
    end
    setfig = figure('name','DFIELD6 Window settings',...
		    'numb','off',...
		    'tag','DFIELD6','vis','off');
  ...
  elseif strcmp(action,'setchange')
    ...
    % on the DFIELD6 Settings window.
  ...
  elseif strcmp(action,'print')
    ...
    nstr = get(dud.notice,'string');
    nstr(1:4) = nstr(2:5);
    nstr{5} = 'Preparing to print the DFIELD6 Display Window.  Please be patient.';
    set(dud.notice,'string',nstr);
    
    nstr(1:4) = nstr(2:5);
    nstr{5} = 'Printing the DFIELD6 Display Window.';
    set(dud.notice,'string',nstr);
  ...
  elseif strcmp(action,'level')
    ...
    xname(find(abs(xname)==92))=[];
    dflevel = findobj('name','DFIELD6 Level sets');
    if ~isempty(dflevel)
      delete(dflevel)
    end
    dflevel = figure('name','DFIELD6 Level sets',...
		     'vis','off',...
		     'numb','off','tag','DFIELD6');
    
    dfield6('figdefault',dflevel);
    hhsetup = get(0,'showhiddenhandles');
  ...
  elseif strcmp(action,'levcomp')
	...
	if isempty(pv) 
	  ...
	  pv(find(abs(pv)==32))=[];
	  lfcn = dfield6('paraeval',pn,pv,lfcn);
	end
      end
    end
    l = length(lfcn);
  ...
  elseif strcmp(action,'showbar')
    ...
    if strcmp(state,'figure')
	  ...
	  'clickedcallback',['dfield6(''zoomback'');' fixtb]);
      set(findobj(sbfig,'tooltipstr','Zoom In'),...
	  'clickedcallback',['dfield6(''zoomin'');' fixtb]);
      set(findobj(sbfig,'tooltipstr','Print'),...
	  'clickedcallback','dfield6(''print'');');
  ...
  elseif strcmp(action,'figdefault')
    ...
    fig = input1;
    set(fig,'CloseRequestFcn','dfield6(''closefcn'')');
    dfset = findobj('name','DFIELD6 Setup');
    sud = get(dfset,'user');
    ud = get(fig,'user');
    ud.ssize = sud.ssize;
  ...
  elseif strcmp(action,'closefcn')
    ...
    fig = gcf;  
    name = get(fig,'name');
    if strcmp(name,'DFIELD6 Setup') | strcmp(name,'DFIELD6 Display')
      quest = ['Closing this window will cause all DFIELD6 ',...
	       'windows to close, and DFIELD6 will stop.  ',...
	       'Do you want to quit DFIELD6?'];
      butt = questdlg(quest,'Quit DFIELD6?','Quit','Cancel','Quit');
      if strcmp(butt,'Quit')
	dfield6('quit');
      end
    elseif strcmp(name,'DFIELD6 Linearization')
      dud = get(fig,'user');
      fcn = dud.function;
	if (exist(fcn)==2) delete([fcn,'.m']);end    
  ...
  elseif strcmp(action,'confused')
    ...
    tstring = 'DFIELD6 is totally confused';
    qstring = {['You will have to restart DFIELD6 from '...
		...
		'it would be best to do nothing now.'];
	       'What do you want to do?'};
    bstr1 = 'Quit and restart DFIELD6.';
    bstr2 = 'Just quit DFIELD6.';
    bstr3 = 'Do nothing.';
    answer = questdlg(qstring,tstring,bstr1,bstr2,bstr3,bstr1);
    if strcmp(answer,bstr1)
      delete(findobj('tag','DFIELD6'));
      dfield6;return
  ...
  elseif strcmp(action,'loadsyst')  % This loads either a system or a gallery.
	...
	errmsg = 'This is not a DFIELD6 file.';
	fprintf('\a')
	errordlg(errmsg,'DFIELD6 error','on');
	return
      end
    ...
    for j = 1:nnn
      ...
      newsyst.name = sname;
      ignore = dfield6('addgall',newsyst);
      if ignore == -1;
