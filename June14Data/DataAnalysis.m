...
totalTime = totalTime - sum(rawData(:, 75));

linearData = sum(rawData);
Data = [linearData(2: 10); linearData(11: 19); linearData(20: 28); linearData(29: 37);...
    linearData(38: 46); linearData(56: 64); linearData(65: 73)];

labels = {'Double Pick', 'Mis-Pick', 'Dropped Box', 'Flipper Jam', 'Phantom Double Pick',...
    'Mis-Label', 'Mis-Flip'};
rootLabels = {'Chute Vision Error', 'Suction Failure', 'Clingy Package', 'Collision', 'Flipper Vision Error',...
...
[misLabel, misLabelLabels, misLabelColors] = filterZeroes(Data(:, 6), rootLabels, colors);
[loosePack, loosePackLabels, loosePackColors] = filterZeroes(Data(:, 7), rootLabels, colors);
...
packsPerMin = rootTotcopy .* 36 / 100;
...
for i = 1:9
    text(i - 0.3, rootTot(i) + 15, num2str(round(packsPerMin(i), 2)), 'VerticalAlignment', 'top', 'FontSize', 14)
end
...
figure;
pie([labelDown - MisRec, MisRec, labelUp]);
t = title('Label Up vs Label Down');
set(gca, 'FontSize', LabelFontSize);
set(t, 'FontSize', TitleFontSize);
legend('Label Down', 'Irrecoverable', 'Label Up');
colormap([0 0.6 0.6; 0.6 0 0.6; 0 0.6 0]);
...
perMinDown = (labelDown - MisRec) / totalTime * 60
missPerMin = MisRec / totalTime * 60
MaxUp = labelUp / (totalTime - unusedTime) * 60
MaxDown = (labelDown - MisRec) / (totalTime - unusedTime) * 60
MaxMissPerMin = MisRec / (totalTime - unusedTime) * 60
% close all;